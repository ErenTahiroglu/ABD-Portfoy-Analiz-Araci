"""
ABD Portföy Analiz Aracı  –  Masaüstü GUI
==========================================
...
try:
    _core = importlib.import_module("ABD_Portföy_Analiz_Aracı")
    HisseAnaliz = _core.HisseAnaliz
except Exception as exc:
    raise ImportError(f"ABD_Portföy_Analiz_Aracı.py yüklenemedi: {exc}") from exc
...
class StdoutRedirector:
    """
    sys.stdout'u kuyruğa yönlendirir.
    Satır içeriğinden sembol ilerleme bilgisi ayıklanır.
    """
    ...
    def _detect(self, line: str) -> None:
        """Print çıktısından ilerleme bilgisi çıkarır."""
        if "Yıllık enflasyon çekiliyor" in line:
            self._q.put(("progress", (0.03, "FRED: enflasyon verisi çekiliyor...")))
        elif "Aylık CPI çekiliyor" in line:
...
class AnalysisThread(threading.Thread):
    """HisseAnaliz'i arka planda çalıştırır, sonuçları Queue ile iletir."""
    ...
    def run(self) -> None:
        ...
        sys.stdout = redir
        try:
            # Alpha Vantage key: GUI'den girilmişse modül değişkenini güncelle
            if self.av_key:
...
class GUIApp(ctk.CTk):
    ...
    def __init__(self):
        super().__init__()
        self.title("ABD Portföy Analiz Aracı  –  v5.0")
    ...
    def _build_ui(self) -> None:
        ...
        self._sym_entry = ctk.CTkEntry(
            sym_f, placeholder_text="AAPL, MSFT, NVDA, VOO, QQQ")
        ...
        self._av_entry = ctk.CTkEntry(
            av_f,
            placeholder_text="ALPHA_VANTAGE_KEY — opsiyonel (ücretsiz: alphavantage.co)",
            show="*")
    ...
    def _populate_table(self, df: pd.DataFrame) -> None:
        ...
        for col in cols:
            ...
            w      = max(len(col) * 9, 100 if is_txt else 80)
        ...
        self._tree.tag_configure("odd",  background="#292929")
        self._tree.tag_configure("even", background="#333333")
    ...
    def _build_chart_tab(self) -> None:
        ...
        self._chart_seg = ctk.CTkSegmentedButton(
            ctrl,
            values=["Yıllık Getiri", "5Y / 3Y", "Aylık Dönemler"],
    ...
    def _style_chart(self) -> None:
        ...
        self._ax.set_facecolor(abg)
        self._ax.tick_params(colors=fg, labelsize=9)
        for attr in ("yaxis", "xaxis"):
            getattr(self._ax, attr).label.set_color(fg)
        self._ax.title.set_color(fg)
        for sp in self._ax.spines.values():
            sp.set_edgecolor(fg)
            sp.set_alpha(0.3)
        self._ax.grid(True, alpha=0.15, color="#888", linestyle="--")
    ...
    def _on_done(self) -> None:
        ...
        if self._current_df is not None:
            ...
            total = sum(len(d) for d in self._session_dfs)
    ...
    def _stop_analysis(self) -> None:
        if self._thread and self._thread.is_alive():
            ...
            self._status_lbl.configure(
                text="Durdurma isteği gönderildi — mevcut sembol bitince durur.")
    ...
    def _save_excel(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile=f"portfoy_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
